...
?>
				...
				<?php
				    include ("../connect.php");
				 	$sql = "SELECT DISTINCT Subject FROM feed_back";
...
<?php
	...
	$sql = "SELECT DISTINCT Subject FROM feed_back order by Subject ASC";
		...
		if ($subdata->num_rows > 0)
		{
		  ...
		  while ($row = $subdata->fetch_object())
		  {
			//get each row to check to see the number of found and not found 
			 if ($data = $conn->query ("SELECT Found FROM feed_back WHERE Subject = '$row->Subject'"))
			 {	
