...
<html lang = "en">
	...
	<div id = "wrapper">
			<div id = "header-div">
				<h1> Visualization Based on Shelves </h1>
			</div>
...
<?php
	function draw ($shelf = '', $chart = '', $yes = '', $no = '', $size = '')
	{
?>		
...
?>
				...
				<?php
				    include ("../connect.php");
				 	$sql = "SELECT DISTINCT ShelfNo FROM feed_back order by ShelfNo";
				 ...
				 ?>
						<tr>
						<td><div id = bar_<?php echo $row->ShelfNo ?>_chart></div></td>
						<td><div id = pie_<?php echo $row->ShelfNo ?>_chart></div></td>
						</tr>
                <?php
					   }
					}
...
<?php
	...
	if ($data = $conn->query ("SELECT Found FROM feed_back"))
	{	
		$yes = 0;
		$no = 0;
		$shelf = 'Total';
		...
		draw($shelf,$chart,$yes,$no,$radius);
	}
		...
		if ($subdata->num_rows > 0)
		{
		  ...
		  while ($row = $subdata->fetch_object())
		  {
			//get each row to check to see the number of found and not found 
			 if ($data = $conn->query ("SELECT Found FROM feed_back WHERE ShelfNo = $row->ShelfNo"))
			 {	
