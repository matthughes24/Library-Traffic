<?php
    ...
    $query = "SELECT * FROM shelf_locations WHERE Map = 2 ORDER BY ShelfNo ASC";
    $query2 = "SELECT * FROM structure_locations WHERE Map = 2 ORDER BY StructID ASC";
    ...
    while($row = mysqli_fetch_array($result)){
        ...
        if(!in_array("MISSING", $list[$i])) {
            ...
            $string = "<svg width=&ldquo;5&rdquo; height=&ldquo;20&rdquo;><a onclick=getBookCase('" .$list[$i]['Bookcase'] . "','a')><rect id =\"" . $list[$i]['ShelfNo'] . "\"  width=\"" . $list[$i]['Width'] . "\" height=\"" . $list[$i]['Height'] . "\" x=\"" . $list[$i]['X'] . "\" y=\"" . $list[$i]['Y'] . "\" style=\"cursor:pointer;\"/></a></svg>";            echo($string);
