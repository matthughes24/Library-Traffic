...
if (isset($_POST['fbSubmit']))
	{
		...
		$query = "INSERT INTO feed_back (CallNo, Found, Time) VALUES ('".$callNo."','".$found."','".$datetime."')";
		
		if ($conn->query($query) === TRUE)
		{
						//determine which shelf that  call number is fall into
			if ($data = $conn->query("SELECT CallNo FROM feed_back"))
			{
				...
				if ($data->num_rows > 0)
				{
				    while ($row = $data->fetch_object())
					{
					   ...
					   $updateShelfNo = "SELECT * FROM book_locations WHERE '$row->CallNo' BETWEEN FIRST AND LAST order by ShelfNo desc limit 1";
					  ...
					  {   
							while ($r = $shelfdata ->fetch_object())
							{
								...
								if(!$conn->query("UPDATE feed_back SET ShelfNo = ($r->ShelfNo)  WHERE CallNo = '$row->CallNo'"))
								{
								    echo "ERROR: ShelfNo can't update in feed_back data table!";
								}
			       ...
			       if (!$conn->query ("UPDATE feed_back SET Subject = '$subject' WHERE $callnopattern ") )
			       {          
