...
<head>
	...
	<link rel="stylesheet" type="text/css" href="f21-Dynamic-Mapping/stylesheets/MapApp.css" media="all">
	<link rel="stylesheet" type="text/css" href="f21-Dynamic-Mapping/stylesheets/LoginModal.css" media="all">
	 <link rel="stylesheet" href="styles/robot.css">
	...
	<script>
			...
			loadDoc("shelvesMain", "f21-Dynamic-Mapping/mainFloorDisplayer.php");
			loadDoc("shelvesConcourse", "f21-Dynamic-Mapping/concourseDisplayer.php");
			loadDoc("shelvesGround", "f21-Dynamic-Mapping/groundDisplayer.php");
    ...
    <script type="text/javascript">
			...
			function getText(callnum) {
				//read text from URL
				var request = new XMLHttpRequest();
				request.open('GET', 'f21-Dynamic-Mapping/bookLocations.php', true);
				...
				request.onreadystatechange = function() {
					if (this.readyState == this.DONE) {
						...
						console.log(callnum);
						callnum = callnum.toUpperCase();
						callnum = callnum.replace('Stacks:', '');
						callnum = callnum.trim();
						//Declaring input call number, parsing to object for useful bits
						var callno = {
							section: callnum.charAt(0),
							subsection: "",
							number: "",
							string: ""
						};
						if (isNaN(callnum.charAt(1))) {
							callno.subsection = callnum.charAt(1);
						}
						for (var i = 1; i < callnum.length; i++) {
							if (isNaN(callnum.charAt(i)) == false && callnum.charAt(i) != '.') {
								callno.number = callno.number + callnum.charAt(i);
							}
							if (callnum.charAt(i) == '.') {
								for (var x = i + 1; x < callnum.length; x++) {
									callno.string = callno.string + callnum.charAt(x);
								}
								break;
							}
						}
			...
			function color(index) {
				...
				for (var i = 0; i < shelvesToColor.length; i++){
					if(shelvesToColor[i] != undefined){
						shelvesToColor[i].setAttribute("style", "fill:green; stroke-width:2; stroke:rgb(0,0,0)");
					}
				}
			}
...
<body>
	...
	<div>
		...
		$servername = "localhost";
			...
			$sql = "SELECT * FROM misplaced_books WHERE book_call_number = '$data' LIMIT 1;";
